...
export default function PriceTicker({ lang }) {
  ...
  const silverG = silver ? (silver / 31.1035).toFixed(2) : '--';
  const silverTL = silver ? ((silver / 31.1035) * rate).toFixed(2) : '--';
  const goldTL = gold ? ((gold / 31.1035) * rate).toFixed(2) : '--';
